// Event listener for loading the Google Sheet
document.getElementById('load-sheet').addEventListener('click', function () {
    ...
    // Make API call to fetch the sheet data
    fetchSheetData(sheetId);
});

// Function to fetch data from Google Sheets
function fetchSheetData(sheetId) {
    const apiUrl = `/api/fetch-sheet?sheetId=${sheetId}`;
...
// Function to display the search results in cards
function displaySearchResults(results) {
    ...
    if (results.length > 0) {
        results.forEach(row => {
            ...
            const cardTitle = row[2] || 'Untitled'; // Column C as the title
                ...
                <div class="col mb-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="card-text">${cardBodyContent}</p> <!-- Columns E to G as body -->
